...
import {
  handleAssetCreatedAssetHubLog,
  handleAssetHubInfoURIChanged,
  handleAssetHubIsOpenChangeLog,
  handleAssetHubUpgradedLog,
  handleAssetUpdatedLog,
  handleCollectedAssetHubLog,
  handleTransferAssetHubLog,
} from './AssetHub';
...
import {
  getAssetHubSet,
  getManager,
  handleAssetHubDeployedLog,
  handleManagerCurationUpdatedLog,
  handleManagerGlobalModuleChangedLog,
  handleManagerHubCreatorNFTChangedLog,
  handleManagerModulesInitializedLog,
} from './AssetManager';
import { ASSETHUB_MANAGER } from '../config';
import { setCurationHanders } from './curation';

export type HandleLogFunc = (
...
export const handlers = new Map<string, Map<string, HandleLogFunc[]>>([
  [
    ASSETHUB_MANAGER!.toLowerCase(),
    new Map([
      [
        assethubManager.events.ModulesInitialized.topic,
        [handleManagerModulesInitializedLog],
      ],
      [
  ...
  [
    ...
    new Map([
      ...
      [assethub.events.InfoURIChanged.topic, [handleAssetHubInfoURIChanged]],
      [assethub.events.IsOpenChanged.topic, [handleAssetHubIsOpenChangeLog]],
      [assethub.events.Collected.topic, [handleCollectedAssetHubLog]],
    ]),
  ],
]);

export const getLogHandler = async (
  ctx: DataHandlerContext<Store>,
  log: Log
) => {
  ...
  if (
    manager?.curation &&
    !handlers.has(manager.curation.toLowerCase()) &&
    manager.curation.toLowerCase() === log.address.toLowerCase()
  ) {
